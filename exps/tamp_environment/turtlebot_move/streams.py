...
from pybullet_tools.utils import get_point, get_custom_limits, all_between, pairwise_collision, \
    ...
    child_link_from_joint, Attachment, OOBB, interpolate_poses, get_oobb_vertices, Pose, AABB, get_aabb, Point
...
class BagOfBoundingBoxes(Environment):
    ...
    def is_motion_valid(self, start, goal):
        ...
        for i in tqdm(range(start.shape[0])):
            start_pose = Pose(Point(x=start[i, 0], y=start[i, 1], z=0))
            goal_pose = Pose(Point(x=goal[i, 0], y=goal[i, 1], z=0))
            for interm_pose in interpolate_poses(start_pose, goal_pose, pos_step_size=0.05):
                # fig, ax = plt.subplots()
                # plt.ylim(-4, 4)
                # plt.xlim(-4, 4)
                interm_oobb = hull(self.oobb_flat_vertices(OOBB(self.robot_shape, interm_pose)))
                ...
                for obstacle in self.obstacles:
                    ...
                    if separating_axis_theorem(interm_oobb, ov):
                        valids[i] = 0
...
def get_anytime_motion_fn(problem, custom_limits={}, collisions=True, teleport=False, holonomic=False, reversible=False, algorithm="prm", num_samples=10, connect_radius=None, **kwargs):
    ...
    def test(rover, q1, q2, fluents=[]):
        ...
        for current_samples in range(100, num_samples, 100):
            prm.grow_to_n_samples(current_samples)
            d, path = prm.query_best_solution(start, goal)
            if(len(path)>0):
                break
